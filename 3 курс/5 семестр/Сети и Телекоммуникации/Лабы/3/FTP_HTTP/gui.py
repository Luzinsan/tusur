...
with dpg.window(label="AUTHORIZATION", modal=True, show=False, tag="auth", no_title_bar=True, autosize=True, ):
    ftp_server_id = dpg.add_input_text(label="Server", default_value="test.rebex.net")
    ftp_port_id = dpg.add_input_int(label="PORT", default_value=DEFAULT_PORT)
    sep = dpg.add_separator()
    login_id = dpg.add_input_text(label="login", default_value="demo")
    password_id = dpg.add_input_text(label="password", default_value="password", password=True)
