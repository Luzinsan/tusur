...
def set_fields(list_info_columns):
    pass
...
def output_columns(sender, table_name):
    ...
    try:
        request = f"SELECT column_name, data_type FROM information_schema.columns WHERE table_name='{table_name}';"
        cursor.execute(request)
