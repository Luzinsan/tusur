...
The wrong sum result occurs because the sum variable is scoped \
...
message is received, resulting in a segmentation fault or abnormal \
termination - depending upon the AIX version. Matching the send data type \
with the receive data type solves the problem.")
}
...
IBM has implemented an eager protocol for messages of a certain size.  \
This subject is discussed in more detail in the MPI Performance Topics \
tutorial.  One possible solution is to include an MPI_Barrier call in  \
the both the send and receive loops.")
}
else if (code == 'mpiex06') {
confirm ("EXPLANATION: \
mpi_bug6 has a bug that will terminate the program under AIX, but be \
ignored under Intel Linux. The problem is that task 2 performs a \
