...
class Server:
    ...
    def listen(self, No_of_conn, buffer, port):
        try:
            ...
            self.write_log(
                self.getTimeStampp() + " Initializing Sockets "
                + " [ready] Binding Sockets [ready] Listening...")
        except:
    ...
    def connection_read_request(self, conn, addr, buffer):
        ...
        try:
            ...
            # Если в заголовке нет порта, то есть, если http-соединение, тогда используйте порт 80, еще порт в заголовке
            webserver = ""
            port = -1
